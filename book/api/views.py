...
class BookSearchByGenreApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not genre:
            return Response(
                {"error": "Genre is required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AddReviewApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not book_id or not user_id or not rating:
            return Response(
                {"error": "book_id, user_id, and rating are required"},
        ...
        try:
            ...
            return Response(
                {"error": "book_id and user_id must be an integer"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
            rating = int(rating)
...
class DeleteReviewApiView(APIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM reviews WHERE book_id = %s AND user_id = %s",
                [book_id, user_id],
            )
            if cursor.fetchone()[0] == 0:
                return Response(
            ...
            try:
                ...
                cursor.execute(
                    "DELETE FROM reviews WHERE book_id = %s AND user_id = %s",
                    [book_id, user_id],
                )
                return Response(
...
class RecommendBooksApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        favoritegenre = (
            ReviewModel.objects.filter(user=user)
            .values("book__genre")
        ...
        if not favoritegenre:
            return Response(
                {"error": "there is not enogh data about you"},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if not suggest_book:
            return Response(
                {"error": "No books found for the favorite genre"},
                status=status.HTTP_404_NOT_FOUND,
            )
